**Language:** [English](README.en.md) | **简体中文**
...
- **五问过滤**：extract 分析前自动过滤掉"Claude 本身已具备的能力"和"一次性需求"，并区分 hook 候选（需要系统层自动触发）、memory 候选（行为偏好）和 CLAUDE.md 候选（始终生效的全局规范），只有真正需要写成 skill 的模式才进入 skill 候选
...
5. **始终生效的全局行为规范**（与项目无关、跨所有 session 适用的硬规则，比 memory 更稳定）→ 分类为 **CLAUDE.md 候选**，单独列出
