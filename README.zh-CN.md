...
- **五问过滤**：extract 分析前自动过滤掉"Claude 本身已具备的能力"和"一次性需求"，并区分 hook 候选（需要系统层自动触发）、memory 候选（行为偏好）和 CLAUDE.md 候选（始终生效的全局规则），只有真正需要写成 skill 的模式才进入 skill 候选
- **skill + hook + memory + CLAUDE.md 四输出**：行为偏好类候选建议写入 memory，需要事件自动触发的模式建议配置为 hook，始终生效的全局行为规范建议写入 CLAUDE.md，均由你确认后落地
...
值得存档的 session  →  零 token 存档     →  跨 session 聚合
   ...
   [record]            distill-logs/        [extract]
                                        ...
                                        全量候选列表（按需求强度排序）
                                           ...
                                           [apply]
                            ↙         ↓          ↓             ↘
                       生成 skill   配置 hook   写入 memory   写入 CLAUDE.md
```
...
AI 在生成候选前会对每个模式做五问过滤，**以下类型不会进入候选列表**：
...
5. **始终生效的全局行为规范**（与项目无关、跨所有 session 无条件适用的硬性规则，比 memory 偏好更接近"约束"，如"禁止擅自 push 到远端"）→ 分类为 **CLAUDE.md 候选**，单独列出
...
所有通过五问过滤的候选**全量保留**，按需求强度排序。四类候选各有独立节，均含落地状态标注：
...
- 全局规则：这条规则的约束内容
- 为什么是 CLAUDE.md 而不是 memory：与 session 无关、与项目无关的硬性规则
- 需求强度：★★★☆☆
```

结果保存至 `~/.claude/distill-logs/_extract_YYYY-MM-DD.md`，包含：Skill / Hook / Memory / CLAUDE.md 候选全量列表、**Analysis Notes**（AI 的分析推理记录）、frontmatter 中的 `applied` / `pending` 结构化状态。多次运行**合并更新**，历史证据不会丢失。
...
extract 中识别为"始终生效全局规则"的模式会单独列出，由你决定是否追加到 `~/.claude/CLAUDE.md`：

```
以下模式更适合写入 CLAUDE.md（硬性规则而非偏好），是否追加？
- 规则 #1: 禁止擅自 push 到远端，必须等用户明确指示
- 规则 #2: commit 中不加 Co-Authored-By，除非用户主动要求
```

确认后追加为 `~/.claude/CLAUDE.md` 中的新节，在所有后续项目的所有 session 中立即生效。
